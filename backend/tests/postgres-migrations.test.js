...
describe('PostgreSQL migration runner seam', () => {
  ...
  it('plans all repository migrations for a fresh PostgreSQL version table without connecting to a database', () => {
    ...
    expect(result).toEqual({
      applied: ['001_foundation', '002_runtime_limits_and_billing', '003_public_api_compatibility', '004_api_key_daily_usage', '005_wallet_nonnegative'],
      skipped: [],
    });
    expect(calls.map(call => call.kind)).toEqual([
      'bootstrap', 'read-applied', 'migration', 'migration', 'migration', 'migration', 'migration',
    ]);
  });

  it('repackages repository migrations without nested outer transactions or placeholder checksums', () => {
    const migrationDirectory = path.resolve(import.meta.dirname, '../migrations/postgres');
    for (const filename of ['001_foundation.sql', '002_runtime_limits_and_billing.sql', '003_public_api_compatibility.sql', '004_api_key_daily_usage.sql', '005_wallet_nonnegative.sql']) {
