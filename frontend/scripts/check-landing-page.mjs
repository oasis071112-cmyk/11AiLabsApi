...
assert.match(landing, /QQ群：575334175/, 'Landing page must show the configured QQ group number')
...
assert.match(landingCss, /grid-template-columns:\s*minmax\(0,\s*1\.25fr\)\s+minmax\(0,\s*0\.9fr\)\s+minmax\(0,\s*1fr\)/, 'Mobile model tabs must fit within the available width')
assert.doesNotMatch(landingCss, /\.model-tabs\s*\{[^}]*overflow-x:\s*auto/s, 'Mobile model tabs must not depend on clipped horizontal scrolling')
