...
const userLayout = source('../src/layouts/UserLayout.vue')
const adminLayout = source('../src/layouts/AdminLayout.vue')
const viteConfig = source('../vite.config.js')

assert.match(viteConfig, /codeSplitting/, '生产构建必须稳定合并首个用户页的核心依赖，避免小 chunk 瀑布')
assert.match(viteConfig, /includeDependenciesRecursively:\s*false/, '依赖分组不能递归吞并懒加载页面')
assert.match(viteConfig, /vendor-vue/, 'Vue 路由与状态核心依赖必须有稳定 chunk')
assert.match(viteConfig, /vendor-icons/, '页面图标必须合并为稳定 chunk')
assert.match(viteConfig, /vendor-element/, 'Element Plus 核心依赖必须有稳定 chunk')
assert.doesNotMatch(app, /onMounted\(\(\)\s*=>\s*authStore\.checkAuth\(\)\)/, '认证校验不能等待根组件挂载后才发起')
assert.match(index, /start\('auth:me','\/api\/auth\/me'\)/, '已有 token 时必须在 Vue 加载前发起认证校验')
assert.match(userLayout, /void authStore\.checkAuth\(\)/, '用户布局必须复用或回退认证校验')
assert.match(adminLayout, /void authStore\.checkAuth\(\)/, '管理布局必须复用或回退认证校验')
assert.match(main, /router\.isReady\(\)/, '路由块解析完成后必须再让出一帧，避免模块解析与首次布局合成长任务')
assert.ok(occurrences(main, 'requestAnimationFrame') >= 2, '应用初始化与首次布局必须分帧执行')
