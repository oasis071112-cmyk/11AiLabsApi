...
<div class="app-shell admin-shell">
  ...
  <aside ref="drawerRef" class="app-sidebar admin-sidebar" :class="{open:drawerOpen,compact:isCompactDesktop}" :aria-hidden="isMobile&&!drawerOpen" :inert="isMobile&&!drawerOpen?'':null">
    <div class="sidebar-mobile-head admin-drawer-head">
      <span>管理菜单</span>
      <button type="button" aria-label="关闭管理菜单" @click="closeDrawer"><X :size="20"/></button>
    </div>
    <div class="admin-logo"><img src="/logo-white.svg?v=ionailabs-20260726" alt="IonAiLabs"/><div>IonAiLabs</div></div>
...
<style scoped>
.admin-sidebar{background:#111827;box-shadow:var(--shadow)}.admin-logo{padding:18px 16px;display:flex;align-items:center;gap:10px;text-align:left;border-bottom:1px solid #1f2937}.admin-logo img{height:28px;margin:0;max-width:34px;object-fit:contain}.admin-logo div{font-size:15px;font-weight:700;color:#fff;letter-spacing:-.02em}.admin-menu{border-right:0;flex:1;min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;padding:10px 8px}.admin-menu :deep(.el-menu-item){height:42px;margin:2px 0;border-radius:9px;transition:transform var(--transition),background var(--transition),color var(--transition)}.admin-menu :deep(.el-menu-item:hover){transform:translateX(2px)}.admin-menu :deep(.el-menu-item.is-active){font-weight:650}.desktop-admin-header{background:#fff;border:1px solid var(--border);border-radius:14px;display:flex;justify-content:space-between;align-items:center;padding:0 20px;height:58px;margin-bottom:10px;flex-shrink:0;box-shadow:var(--shadow-sm)}.desktop-admin-header>span{font-size:16px;font-weight:700;color:#000;letter-spacing:-.02em}.desktop-admin-header>div{display:flex;align-items:center;gap:12px}.desktop-admin-header small{font-size:12px;color:var(--text-muted);font-weight:500}.desktop-admin-header strong{font-size:13px;font-weight:600;font-variant-numeric:tabular-nums}.admin-mobile-account{display:none}
@media(min-width:769px) and (max-width:1180px){.admin-sidebar.compact{width:68px}.admin-sidebar.compact .admin-logo{padding:18px 0;justify-content:center}.admin-sidebar.compact .admin-logo img{height:28px;margin:0}.admin-sidebar.compact .admin-logo div{display:none}.admin-sidebar.compact .admin-menu{width:68px;padding:10px 8px}.admin-sidebar.compact :deep(.el-menu-item){padding:0!important;justify-content:center;font-size:0}.admin-sidebar.compact :deep(.el-menu-item .el-icon){margin:0;font-size:16px}}
