...
<div class="app-shell admin-shell">
  <header class="mobile-topbar admin-mobile-topbar">
    <button class="mobile-menu-button" type="button" aria-label="打开管理菜单" @click="drawerOpen=true"><Menu :size="22"/></button>
    <div class="mobile-page-title"><small>管理后台</small><span>{{ pageTitle }}</span></div>
    <button class="mobile-avatar admin-avatar" type="button" aria-label="打开管理菜单" @click="drawerOpen=true">{{ userInitial }}</button>
  </header>

  <button v-if="drawerOpen" class="drawer-backdrop" type="button" aria-label="关闭管理菜单" @click="drawerOpen=false"></button>

  <aside class="app-sidebar admin-sidebar" :class="{open:drawerOpen}">
    <div class="sidebar-mobile-head admin-drawer-head">
      <span>管理菜单</span>
      <button type="button" aria-label="关闭管理菜单" @click="drawerOpen=false"><X :size="20"/></button>
    </div>
    <div class="admin-logo"><img src="/logo-white.svg" alt="11AiLabs"/><div>11AiLabs</div></div>
    <el-menu :default-active="route.path" background-color="#111827" text-color="#9ca3af" active-text-color="#C5D5C0" router class="admin-menu" @select="drawerOpen=false">
...
const route=useRoute(),router=useRouter(),authStore=useAuthStore(),drawerOpen=ref(false)
const menuItems=[
