...
async function showDetail(u){detailUser.value=u;detailDialog.value=true}
async function doAdjust(){if(!adj.value.amount)return ElMessage.warning('请输入点数');adjusting.value=true;try{await api.post(`/api/admin/users/${detailUser.value.id}/adjust-balance`,adj.value);ElMessage.success('调账成功');adjustDialog.value=false;showDetail(detailUser.value)}catch(e){}adjusting.value=false}
