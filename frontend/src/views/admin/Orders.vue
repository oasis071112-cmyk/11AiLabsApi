...
onMounted(()=>fetch())
async function fetch(){loading.value=true;const p={page:page.value};if(statusFilter.value)p.status=statusFilter.value;try{const r=await api.get('/api/admin/recharge-orders',{params:p});orders.value=r.data.data;total.value=r.data.pagination.total}catch(e){ElMessage.error(e.response?.data?.error||'订单加载失败，请重试')}loading.value=false}
...
function st(s){const m={pending:'warning',paid:'primary',granted:'success',credited:'success',rejected:'danger',cancelled:'info',abnormal:'danger'};return m[s]||'info'}
function sl(s){const m={pending:'待支付',paid:'已支付',granted:'已发放',credited:'已发放',rejected:'已驳回',cancelled:'已取消',abnormal:'异常'};return m[s]||s}
</script>
