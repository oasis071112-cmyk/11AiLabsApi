...
<div class="admin-dashboard admin-page">
  ...
  <section v-if="!dashboardLoading" class="admin-insights">
...
const data=ref({}),isMobile=useMobile(),dashboardLoading=ref(true),trendRoot=ref(null),trendVisible=ref(false)
...
onMounted(async()=>{try{data.value=(await api.get('/api/admin/dashboard/bootstrap')).data}catch(e){try{data.value=(await api.get('/api/admin/dashboard')).data}catch(_fallbackError){}}finally{dashboardLoading.value=false}})
