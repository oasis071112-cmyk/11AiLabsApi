...
<el-dialog v-model="providerDialog" :title="providerForm.id?'编辑易支付':'新增易支付'" width="560px"><el-form :model="providerForm" label-width="128px"><el-form-item label="服务商名称"><el-input v-model="providerForm.provider_name"/></el-form-item><el-form-item label="API 地址"><el-input v-model="providerForm.api_base_url" placeholder="https://pay.example.com"/></el-form-item><el-form-item label="商户 PID"><el-input v-model="providerForm.merchant_id"/></el-form-item><el-form-item label="商户 PKey"><el-input v-model="providerForm.merchant_key" type="password" show-password :placeholder="providerForm.id?'留空则保留原 PKey':'易支付商户密钥'"/></el-form-item><el-form-item label="支付宝通道 ID"><el-input v-model="providerForm.alipay_type" placeholder="留空使用 alipay"/></el-form-item><el-form-item label="微信通道 ID"><el-input v-model="providerForm.wechat_type" placeholder="留空使用 wxpay"/></el-form-item><el-form-item label="状态"><el-switch v-model="providerForm.status" active-value="active" inactive-value="inactive" active-text="启用" inactive-text="停用"/></el-form-item></el-form><template #footer><el-button @click="providerDialog=false">取消</el-button><el-button type="primary" :loading="providerSaving" @click="saveProvider">保存</el-button></template></el-dialog>
</div>
</template>
...
const freshProvider=()=>({id:null,provider_name:'易支付',api_base_url:'',merchant_id:'',merchant_key:'',alipay_type:'',wechat_type:'',status:'inactive'})
...
function openProvider(row){Object.assign(providerForm,freshProvider(),row?{...row,merchant_key:''}:{});providerDialog.value=true}
