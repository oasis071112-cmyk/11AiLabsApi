...
<div class="routing-page admin-page">
  ...
  <section class="section-card">
    ...
    <el-table :data="activeChannels" stripe v-loading="channelLoading">
      ...
      <el-table-column label="计费倍率" min-width="180"><template #default="{row}"><span :class="channelMultiplierSummary(row)==='全局规则'?'muted':''">{{ channelMultiplierSummary(row) }}</span></template></el-table-column>
  ...
  <el-dialog v-model="channelDialog" :title="editingChannel?'编辑上游渠道':'新增上游渠道'" width="520px">
    <el-form :model="channelForm" label-width="100px">
      ...
      <el-divider content-position="left">渠道计费倍率</el-divider>
      <el-alert title="填写后覆盖原有用户/平台规则或模型倍率；留空则完全沿用原有规则。不会新增订阅或改变点数扣费方式。" type="info" :closable="false" style="margin-bottom:12px"/>
      <div class="channel-multiplier-grid">
        <el-form-item label="输入倍率"><el-input-number v-model="channelForm.billing_multiplier_input" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/><div class="switch-help">留空：全局规则</div></el-form-item>
        <el-form-item label="输出倍率"><el-input-number v-model="channelForm.billing_multiplier_output" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/><div class="switch-help">留空：全局规则</div></el-form-item>
        <el-form-item label="图片倍率"><el-input-number v-model="channelForm.billing_multiplier_image" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/><div class="switch-help">留空：全局规则</div></el-form-item>
      </div>
    </el-form>
...
const emptyChannel=()=>({channel_name:'',base_url:'',api_key:'',priority:0,weight:100,protocol_type:'openai_compatible',capabilities:['chat_completions'],billing_multiplier_input:null,billing_multiplier_output:null,billing_multiplier_image:null})
...
function channelMultiplierSummary(row){const pairs=[['输入',row.billing_multiplier_input],['输出',row.billing_multiplier_output],['图片',row.billing_multiplier_image]].filter(([,value])=>value!==null&&value!==undefined&&value!=='');return pairs.length?pairs.map(([label,value])=>`${label} ${Number(value).toFixed(2)}`).join(' · '):'全局规则'}
</script>

<style scoped>
.routing-page{padding-bottom:24px}.page-hint,.section-head p{font-size:12px;color:#94a3b8;margin:4px 0 0}.section-card{background:#fff;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:18px;overflow:hidden}.section-head{padding:16px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #eef2f7}.section-head h4{margin:0;color:#0f172a}.mr-tag{margin:2px 4px 2px 0}.muted,.switch-help{color:#94a3b8;font-size:12px}.switch-help{margin-left:10px}.inactive-section{border-top:1px solid #eef2f7}.inactive-section :deep(.el-collapse-item__header){padding:0 18px;font-size:13px;font-weight:600;color:#64748b}.inactive-section :deep(.el-collapse-item__content){padding:0}.collapse-hint{margin-left:auto;margin-right:12px;font-size:12px;font-weight:400;color:#94a3b8}.billing-editor{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;padding:14px 18px;background:#f8fafc}.billing-editor label{display:flex;flex-direction:column;gap:5px;font-size:12px;color:#64748b}.billing-editor :deep(.el-input-number),.billing-editor :deep(.el-select){width:100%}.channel-multiplier-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0 12px}.channel-multiplier-grid :deep(.el-form-item){display:block;margin-bottom:12px}.channel-multiplier-grid :deep(.el-form-item__label){display:block;float:none;width:auto!important;height:auto;line-height:20px;padding:0;text-align:left}.channel-multiplier-grid :deep(.el-form-item__content){display:block;margin-left:0!important;line-height:normal}.channel-multiplier-grid :deep(.el-input-number){width:100%}
@media(max-width:768px){.section-card{margin-bottom:12px}.section-head{padding:14px;align-items:flex-start;gap:12px;flex-wrap:wrap}.section-head .el-button{width:100%;margin:0}.switch-help{display:block;margin:6px 0 0}.channel-multiplier-grid{grid-template-columns:1fr}.routing-page :deep(.el-table__inner-wrapper){min-width:680px}.routing-page :deep(.el-dialog .el-table__inner-wrapper){min-width:620px}}
</style>
