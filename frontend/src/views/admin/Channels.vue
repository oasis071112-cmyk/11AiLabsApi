...
async function saveMappings(){savingMappings.value=true;try{const models=mappingRows.value.filter(item=>item.selected||item.hasMapping).map(item=>({model_code:item.model_code,upstream_model_name:item.upstream_model_name||item.model_code,supports_image_input:Boolean(item.supports_image_input),status:item.selected?'active':'inactive',...Object.fromEntries(billingFields.map(field=>[field,item[field]]))}));await api.put(`/api/admin/channels/${mappingChannel.value.id}/models`,{models});ElMessage.success('模型映射与计费已保存');mappingDialog.value=false;await loadAll()}finally{savingMappings.value=false}}
