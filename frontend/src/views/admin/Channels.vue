...
<div class="routing-page admin-page">
  ...
  <section class="section-card">
    ...
    <el-table :data="activeGroups" stripe v-loading="groupLoading">
      ...
      <el-table-column label="用户扣费倍率" min-width="210"><template #default="{row}">{{ groupMultiplierSummary(row) }}</template></el-table-column>
  ...
  <section class="section-card">
    ...
    <el-table :data="activeChannels" stripe v-loading="channelLoading">
      ...
      <el-table-column label="接口能力" min-width="300"><template #default="{row}"><el-tag v-if="row.capabilities?.includes('chat_completions')" size="small" effect="plain" class="mr-tag">对话</el-tag><el-tag v-if="row.capabilities?.includes('embeddings')" size="small" effect="plain" type="success" class="mr-tag">向量</el-tag><el-tag v-if="row.capabilities?.includes('image_generations')" size="small" effect="plain" type="warning" class="mr-tag">生图</el-tag><el-tag v-if="row.capabilities?.includes('image_edits')" size="small" effect="plain" type="warning" class="mr-tag">编辑</el-tag><el-tag v-if="row.capabilities?.includes('image_variations')" size="small" effect="plain" type="warning" class="mr-tag">变体</el-tag><el-tag v-if="row.capabilities?.includes('image_transformations')" size="small" effect="plain" type="warning" class="mr-tag">变换</el-tag><el-tag v-if="row.capabilities?.includes('responses')" size="small" effect="plain" type="primary" class="mr-tag">Responses</el-tag><el-tag v-if="row.capabilities?.includes('anthropic_messages')" size="small" effect="plain" type="warning" class="mr-tag">Messages</el-tag><el-tag v-if="row.capabilities?.includes('anthropic_count_tokens')" size="small" effect="plain" type="info" class="mr-tag">Count Tokens</el-tag></template></el-table-column>
  ...
  <el-dialog v-model="groupDialog" :title="editingGroup?'编辑路由分组':'新增路由分组'" width="760px">
    <el-form :model="groupForm" label-width="100px">
      ...
      <el-divider content-position="left">用户扣费倍率</el-divider>
      <el-alert title="扣费顺序：单独用户倍率 → 路由分组倍率 → 全局倍率 → 1×。留空表示沿用全局倍率。" type="info" :closable="false" style="margin-bottom:12px"/>
      <div class="group-multiplier-grid">
        <el-form-item label="输入倍率"><el-input-number v-model="groupForm.billing_multiplier_input" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/></el-form-item>
        <el-form-item label="输出倍率"><el-input-number v-model="groupForm.billing_multiplier_output" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/></el-form-item>
        <el-form-item label="图片倍率"><el-input-number v-model="groupForm.billing_multiplier_image" :min="0.000001" :step="0.1" :value-on-clear="null" controls-position="right"/></el-form-item>
      </div>
  ...
  <el-dialog v-model="channelDialog" :title="editingChannel?'编辑上游渠道':'新增上游渠道'" width="520px">
    <el-form :model="channelForm" label-width="100px">
      ...
      <el-form-item label="协议" required><el-select v-model="channelForm.protocol_type" @change="changeChannelProtocol"><el-option value="openai_compatible" label="OpenAI 兼容协议"/><el-option value="anthropic" label="Anthropic Messages 原生协议"/></el-select></el-form-item>
      ...
      <el-form-item label="接口能力" required><div><el-checkbox-group v-if="channelForm.protocol_type==='anthropic'" v-model="channelForm.capabilities"><el-checkbox value="anthropic_messages">Messages</el-checkbox><el-checkbox value="anthropic_count_tokens">Count Tokens</el-checkbox></el-checkbox-group><el-checkbox-group v-else v-model="channelForm.capabilities"><el-checkbox value="chat_completions">对话 / Chat Completions</el-checkbox><el-checkbox value="embeddings">向量 / Embeddings</el-checkbox><el-checkbox value="image_generations">生图 / Images Generations</el-checkbox><el-checkbox value="image_edits">图片编辑 / Images Edits</el-checkbox><el-checkbox value="image_variations">图片变体 / Images Variations</el-checkbox><el-checkbox value="image_transformations">图片变换 / Images Transformations</el-checkbox><el-checkbox value="responses">Responses 原生工具</el-checkbox></el-checkbox-group><div class="switch-help">只会把请求分配给协议与接口能力都匹配的渠道，不进行跨协议转换。</div></div></el-form-item>
      <el-form-item label="最大并发"><el-input-number v-model="channelForm.max_concurrency" :min="1" :max="1000"/><span class="switch-help">同一渠道可同时处理的上游请求数，新建默认为 5。</span></el-form-item>
...
onMounted(()=>{void loadGroups();void loadChannels()})
async function loadAll(){await Promise.all([loadGroups(),loadChannels()])}
...
async function ensureModels(){
  ...
  if(!modelsRequest){
    modelsRequest=api.get('/api/admin/models').then(result=>{
...
function openGroup(row){editingGroup.value=row||null;const linked=new Map((row?.channels||[]).map(item=>[String(item.account_id??item.channel_id),item]));groupForm.value={group_name:row?.group_name||'',description:row?.description||'',fallback_group_id:row?.fallback_group_id||null,status:row?.status||'active',restrict_models:Number(row?.restrict_models||0)===1,model_codes:[...(row?.model_codes||[])],model_rules:(row?.model_rules||[]).map(rule=>({...rule})),billing_multiplier_input:row?.billing_multiplier_input??null,billing_multiplier_output:row?.billing_multiplier_output??null,billing_multiplier_image:row?.billing_multiplier_image??null,channels:channels.value.map(channel=>{const link=linked.get(String(channel.id));return{channel_id:channel.id,channel_name:channel.channel_name,health_score:channel.health_score,selected:!!link,priority:link?.priority??channel.priority??0,weight:link?.weight??channel.weight??100}})};groupDialog.value=true;void ensureModels().catch(()=>{})}
async function saveGroup(){if(!groupForm.value.group_name.trim()){ElMessage.warning('请输入分组名称');return}savingGroup.value=true;try{const existingRules=new Map((groupForm.value.model_rules||[]).map(rule=>[rule.model_code,rule]));const model_rules=(groupForm.value.model_codes||[]).map(model_code=>({...existingRules.get(model_code),model_code,status:'active'}));const payload={...groupForm.value,model_rules,channels:groupForm.value.channels.filter(item=>item.selected).map(({channel_id,priority,weight})=>({channel_id,priority,weight,status:'active'}))};if(editingGroup.value)await api.put(`/api/admin/routing-groups/${editingGroup.value.id}`,payload);else await api.post('/api/admin/routing-groups',payload);ElMessage.success('分组已保存');groupDialog.value=false;await loadGroups()}finally{savingGroup.value=false}}
...
const emptyChannel=()=>({channel_name:'',base_url:'',api_key:'',max_concurrency:5,priority:0,weight:100,protocol_type:'openai_compatible',capabilities:['chat_completions']})
...
function changeChannelProtocol(protocol){channelForm.value.capabilities=protocol==='anthropic'?['anthropic_messages','anthropic_count_tokens']:['chat_completions']}
...
async function syncModels(row){syncingId.value=row.id;try{const result=await api.post(`/api/admin/channels/${row.id}/sync-models`);models.value=[];ElMessage.success(result.data.message);await loadAll()}finally{syncingId.value=null}}
...
async function openMappings(row){mappingChannel.value=row;mappingDialog.value=true;mappingLoading.value=true;mappingSearch.value='';try{const [catalog,result]=await Promise.all([ensureModels(),api.get(`/api/admin/channels/${row.id}/models`)]);const payload=result.data||{};const mappings=payload.mappings||payload.data||[];const byCode=new Map(mappings.map(item=>[item.model_code,{...item,...(item.configuration||{})}]));mappingRows.value=(payload.mappings?payload.data:catalog).map(model=>{const mapping=byCode.get(model.model_code);return{...model,...Object.fromEntries(billingFields.map(field=>[field,mapping?.[field]??(field==='billing_model_source'?'channel_mapped':field==='billing_mode'?'':null)])),hasMapping:Boolean(mapping),selected:mapping?.status==='active',upstream_model_name:mapping?.upstream_model_name||model.upstream_model_name||model.model_code,supports_image_input:mapping?.supports_image_input==null?Boolean(model.is_multimodal):Number(mapping.supports_image_input)===1}})}finally{mappingLoading.value=false}}
async function saveMappings(){savingMappings.value=true;try{const models=mappingRows.value.filter(item=>item.selected||item.hasMapping).map(item=>{const configuration=Object.fromEntries(billingFields.map(field=>[field,item[field]]));return{model_code:item.model_code,upstream_model_name:item.upstream_model_name||item.model_code,supports_image_input:Boolean(item.supports_image_input),status:item.selected?'active':'inactive',...configuration,configuration}});await api.put(`/api/admin/channels/${mappingChannel.value.id}/models`,{models});ElMessage.success('模型映射与计费已保存');mappingDialog.value=false;await loadAll()}finally{savingMappings.value=false}}
...
function protocolLabel(protocol){return protocol==='anthropic'?'Anthropic':'OpenAI 兼容'}
function groupMultiplierSummary(row){return [['入',row.billing_multiplier_input],['出',row.billing_multiplier_output],['图',row.billing_multiplier_image]].map(([label,value])=>`${label} ${value===null||value===undefined||value===''?'全局':`×${Number(value).toFixed(2)}`}`).join(' · ')}
</script>

<style scoped>
.routing-page{padding-bottom:24px}.page-hint,.section-head p{font-size:12px;color:#94a3b8;margin:4px 0 0}.section-card{background:#fff;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:18px;overflow:hidden}.section-head{padding:16px 18px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #eef2f7}.section-head h4{margin:0;color:#0f172a}.mr-tag{margin:2px 4px 2px 0}.muted,.switch-help{color:#94a3b8;font-size:12px}.switch-help{margin-left:10px}.inactive-section{border-top:1px solid #eef2f7}.inactive-section :deep(.el-collapse-item__header){padding:0 18px;font-size:13px;font-weight:600;color:#64748b}.inactive-section :deep(.el-collapse-item__content){padding:0}.collapse-hint{margin-left:auto;margin-right:12px;font-size:12px;font-weight:400;color:#94a3b8}.billing-editor{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;padding:14px 18px;background:#f8fafc}.billing-editor label{display:flex;flex-direction:column;gap:5px;font-size:12px;color:#64748b}.billing-editor :deep(.el-input-number),.billing-editor :deep(.el-select){width:100%}.group-multiplier-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0 12px}.group-multiplier-grid :deep(.el-form-item){display:block;margin-bottom:12px}.group-multiplier-grid :deep(.el-form-item__label){display:block;float:none;width:auto!important;height:auto;line-height:20px;padding:0;text-align:left}.group-multiplier-grid :deep(.el-form-item__content){display:block;margin-left:0!important;line-height:normal}.group-multiplier-grid :deep(.el-input-number){width:100%}
@media(max-width:768px){.section-card{margin-bottom:12px}.section-head{padding:14px;align-items:flex-start;gap:12px;flex-wrap:wrap}.section-head .el-button{width:100%;margin:0}.switch-help{display:block;margin:6px 0 0}.group-multiplier-grid{grid-template-columns:1fr}.routing-page :deep(.el-table__inner-wrapper){min-width:680px}.routing-page :deep(.el-dialog .el-table__inner-wrapper){min-width:620px}}
</style>
