...
<div class="models-page admin-page">
  ...
  <el-dialog v-model="dialogVisible" :title="isEdit?'编辑模型':'新增模型'" width="650px"><el-form :model="form" label-width="130px">
    ...
    <template v-if="form.model_type==='image'">
      <el-alert title="活动图片模型必须完整配置 1K、2K、4K 三档正数价格" type="info" :closable="false" description="渠道对应档位留空时使用这里的模型同档价格；缺价时禁止启用和计费。"/>
      ...
      <el-form-item label="1K 单张价格"><el-input-number v-model="form.official_image_price_1k" :min="0.000001" :precision="6" :step="0.001"/><div class="form-help">适用于最长边不超过 1024px 的图片。</div></el-form-item>
      <el-form-item label="2K 单张价格"><el-input-number v-model="form.official_image_price_2k" :min="0.000001" :precision="6" :step="0.001"/><div class="form-help">适用于最长边不超过 2048px 的图片。</div></el-form-item>
      <el-form-item label="4K 单张价格"><el-input-number v-model="form.official_image_price_4k" :min="0.000001" :precision="6" :step="0.001"/><div class="form-help">适用于最长边超过 2048px 的图片。</div></el-form-item>
    </template>
    <template v-else>
...
const emptyForm=()=>({model_code:'',model_name:'',upstream_model_name:'',model_type:'llm',context_length:4096,is_multimodal:false,multiplier_input:1,multiplier_output:1,multiplier_image:1,official_provider:activeProvider.value,official_model_id:'',official_pricing_mode:'auto',official_currency:activeProvider.value==='deepseek'?'CNY':'USD',official_input_price:0,official_cached_input_price:0,official_output_price:0,official_image_price_1k:null,official_image_price_2k:null,official_image_price_4k:null,status:'active',sort_order:0})
const form=ref(emptyForm())
onMounted(fetchModels)
const asMultimodal=value=>value===true||Number(value)===1
async function fetchModels(){loading.value=true;try{models.value=((await api.get('/api/admin/models')).data.data||[]).map(model=>({...model,is_multimodal:asMultimodal(model.is_multimodal)}))}catch(e){ElMessage.error(e.response?.data?.error||'模型列表加载失败，请重试')}loading.value=false}
function imagePrices(value){try{return typeof value==='string'?JSON.parse(value||'{}'):(value||{})}catch(e){return {}}}
function openDialog(row){isEdit.value=!!row;if(!row){form.value=emptyForm()}else{const prices=imagePrices(row.official_image_prices);form.value={...row,is_multimodal:asMultimodal(row.is_multimodal),official_pricing_mode:row.official_pricing_mode||'auto',multiplier_input:row.billing_multiplier_input,multiplier_output:row.billing_multiplier_output,multiplier_image:row.billing_multiplier_image||1,official_image_price_1k:prices['1K']??null,official_image_price_2k:prices['2K']??null,official_image_price_4k:prices['4K']??null}}dialogVisible.value=true}
async function save(){if(form.value.model_type==='image'&&form.value.status==='active'&&!imageTiers.every(tier=>Number(form.value[`official_image_price_${tier.toLowerCase()}`])>0)){ElMessage.warning('活动图片模型必须完整填写 1K、2K、4K 三档正数价格');return}saving.value=true;try{if(isEdit.value)await api.put(`/api/admin/models/${form.value.id}`,form.value);else await api.post('/api/admin/models',form.value);ElMessage.success('保存成功');dialogVisible.value=false;fetchModels()}catch(e){ElMessage.error(e.response?.data?.error||'模型保存失败')}saving.value=false}
...
async function syncPricing(){if(syncing.value)return;syncing.value=true;ElMessage.info('同步任务已开始，正在读取官方价格与汇率…');try{const r=await api.post('/api/admin/pricing-sync',{}, {timeout:300000});ElMessage.success(`同步完成：更新 ${r.data.official_pricing.updated} 个模型，USD/CNY=${r.data.exchange_rate.rate}`);fetchModels()}catch(e){ElMessage.error(e.response?.data?.error||'官方价格和汇率同步失败')}finally{syncing.value=false}}
function currency(value){return value==='USD'?'$':'¥'}
function hasCompleteImagePrices(row){const prices=imagePrices(row.official_image_prices);return imageTiers.every(tier=>Number(prices[tier])>0)}
function imagePrice(row,tier){const value=Number(imagePrices(row.official_image_prices)[tier]);return value>0?`${currency(row.official_currency||'USD')}${value.toFixed(6)}`:'—'}
