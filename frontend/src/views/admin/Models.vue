...
<div class="models-page admin-page">
  ...
  <div class="table-card" v-loading="loading">
    <el-table class="desktop-model-table" :data="activeModels" stripe empty-text="该分类暂无已上架模型">
      <el-table-column type="expand" width="52"><template #default="{row}"><div class="channel-mapping-list"><div class="mapping-title">渠道来源与映射状态</div><article v-for="mapping in row.channel_mappings" :key="mapping.channel_id" class="channel-mapping-row"><div><strong>{{ mapping.channel_name }}</strong><span>{{ mapping.upstream_model_name }}</span><div class="mapping-groups"><el-tag v-for="group in mapping.routing_group_names" :key="group" size="small" effect="plain">{{ group }}</el-tag><em v-if="mapping.channel_status!=='active'">渠道已停用</em></div></div><div class="mapping-rates"><span>输入 {{ mappingRate(mapping,'input') }}</span><span>输出 {{ mappingRate(mapping,'output') }}</span><span>图片 {{ mappingRate(mapping,'image') }}</span></div><el-switch :model-value="mapping.status==='active'" :disabled="mapping.channel_status!=='active'&&mapping.status!=='active'" @change="toggleMapping(row,mapping)"/></article><el-empty v-if="!row.channel_mappings?.length" description="暂无渠道映射，请先在渠道管理中添加" :image-size="44"/></div></template></el-table-column>
    ...
    <div class="mobile-model-list">
      <article v-for="row in activeModels" :key="row.id" class="mobile-model-card">
        ...
        <div class="mobile-channel-mappings"><div class="mapping-title">渠道来源</div><article v-for="mapping in row.channel_mappings" :key="mapping.channel_id"><div><strong>{{ mapping.channel_name }}</strong><small>{{ mapping.routing_group_names?.join('、')||'未加入号池' }}</small><span>图 {{ mappingRate(mapping,'image') }}</span></div><el-switch :model-value="mapping.status==='active'" :disabled="mapping.channel_status!=='active'&&mapping.status!=='active'" @change="toggleMapping(row,mapping)"/></article><span v-if="!row.channel_mappings?.length" class="pending">暂无渠道映射</span></div>
  ...
  <el-dialog v-model="dialogVisible" :title="isEdit?'编辑模型':'新增模型'" width="650px"><el-form :model="form" label-width="130px">
    ...
    <el-alert v-if="form.model_type==='image'" title="图片生成统一采用默认兜底价格" type="info" :closable="false" description="不设置官方价格或手动价格；实际结算仍按图片尺寸、渠道配置和图片倍率计算。"/>
    <template v-else>
...
function mappingRate(mapping,dimension){const value=mapping[`billing_multiplier_${dimension}`];return value===null||value===undefined||value===''?'沿用平台':`×${Number(value).toFixed(2)}`}
...
.channel-mapping-list{padding:14px 24px;background:#f8fafc}.mapping-title{font-size:12px;font-weight:700;color:#64748b;margin-bottom:10px}.channel-mapping-row{display:grid;grid-template-columns:minmax(210px,1fr) minmax(280px,1fr) auto;gap:18px;align-items:center;padding:12px 14px;background:#fff;border:1px solid #e5e7eb;border-radius:10px;margin-bottom:8px}.channel-mapping-row>div:first-child{display:flex;flex-direction:column;gap:3px}.channel-mapping-row strong{color:#0f172a}.channel-mapping-row span{font-size:12px;color:#64748b}.mapping-groups{display:flex;gap:5px;flex-wrap:wrap;align-items:center}.mapping-groups em{font-style:normal;color:#dc2626;font-size:12px}.mapping-rates{display:flex;gap:12px;flex-wrap:wrap}.mapping-rates span{white-space:nowrap}.mobile-channel-mappings{border-top:1px solid #f1f5f9;margin-top:10px;padding-top:10px}.mobile-channel-mappings article{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 0}.mobile-channel-mappings article+article{border-top:1px solid #f1f5f9}.mobile-channel-mappings article>div{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:3px 10px;min-width:0}.mobile-channel-mappings article strong{color:#0f172a}.mobile-channel-mappings article small{grid-column:1;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.mobile-channel-mappings article span{grid-column:2;grid-row:1/3;align-self:center;color:#2563eb;font-size:12px}.single-action{grid-template-columns:1fr!important}
