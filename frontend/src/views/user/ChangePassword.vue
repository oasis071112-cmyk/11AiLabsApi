...
async function submit(){const ok=await formRef.value.validate().catch(()=>false);if(!ok)return;loading.value=true;try{await api.put('/api/auth/password',{oldPassword:form.value.oldPassword,newPassword:form.value.newPassword});ElMessage.success('密码修改成功');formRef.value.resetFields()}catch(e){ElMessage.error(e.response?.data?.error||'修改失败')}loading.value=false}
</script>
<style scoped>
