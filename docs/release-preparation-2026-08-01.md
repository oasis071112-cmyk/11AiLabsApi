...
| `005_wallet_nonnegative` | `4dfb0ef7d5147f47863ec764b949f6c69e21722dd0e5e5afa317cd80679e4e83` |
...
- 上游实际返回 1254x1254，系统按实际输出归入 2K 档计费。发布覆盖会同时锁定模型目录与账号映射中的 1K/2K/4K 三档价格为 USD 0.20，防止旧映射价格抢占优先级。
- Redis 断开时网关在 116–130ms 内返回 HTTP 503 `redis_unavailable`，未产生冻结余额；恢复 Redis 后 API 和 worker 无需重启即可重新达到 ready。

运行决策：多图请求按每张图片的实际输出档位分别结算。若最终费用超过当前可结算资金，不再生成负余额；保留原冻结并将请求标记为 `pending_review`，由管理员核对。PostgreSQL 已恢复钱包四个金额字段的非负约束。

## 测试与性能

- 后端全量：57 个测试文件、348 项测试通过；2 个真实 PostgreSQL 集成文件在普通全量命令中按环境变量跳过，已单独连接演练库通过。
- 前端生产构建通过；首屏预算、性能垂直切片、登录 bootstrap、日志筛选防旧响应覆盖、HTTP 取消与渠道兼容契约均通过。
- Playwright：用户首屏核心数据 929ms、管理员首屏 866ms；管理仪表盘 500 会在 588ms 内结束骨架屏并显示错误与重试；日志 A→B 筛选与旧接口 fallback 均只保留最新响应。6 项通过，1 项需要真实环境的用例按配置跳过。此外，真实 API + PostgreSQL + Redis 管理员登录及全部管理页面验收通过，9.5 秒完成全页面巡检，未观察到管理接口 5xx。
- Lighthouse：Performance 100、Accessibility 100、Best Practices 100、SEO 92；FCP 471ms、LCP 686ms、CLS 0.000。
- 前端 `dist/index.html` SHA-256：`79b722c1dde240c7ab4c498926b58ceb117a71bc896fddb402bb3478bad19155`。
...
- 图片结算会识别返回图片的实际尺寸，混合尺寸多图分别计算 1K/2K/4K 小计，不会把全部图片按最大档收费；安全快照保留原始输出分布与实际收费分档。
- 控制面导入期间强制关闭支付、注册与新用户赠送；完成用户面清零和验收后，再根据最新线上快照显式恢复原开关。
- 支付成功回调经过正确验签和金额校验后，即使到达时订单已超过本地等待期，也会幂等入账；回调 URL 不写入 HTTP access log。
