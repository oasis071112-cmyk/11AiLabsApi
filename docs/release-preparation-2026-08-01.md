...
运行决策：多图请求统一按本请求已确认的最高输出档位结算。若最终费用超过当前可结算资金，不再生成负余额；保留原冻结并将请求标记为 `pending_review`，由管理员核对。PostgreSQL 已恢复钱包四个金额字段的非负约束。
...
- 图片结算会识别返回图片的实际尺寸，混合尺寸多图按本请求已确认的最高档统一计费，并在安全快照中分别保留原始输出分布与实际收费档。
