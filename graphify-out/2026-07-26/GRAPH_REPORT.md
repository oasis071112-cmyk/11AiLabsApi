# Graph Report - ai-api-proxy  (2026-07-26)

## Corpus Check
- 107 files · ~57,569 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1050 nodes · 1492 edges · 82 communities (69 shown, 13 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 84 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `8b745963`
...
1. `getDatabase()` - 34 edges
2. `handleImageBilledRequest()` - 32 edges
3. `initDatabase()` - 27 edges
4. `saveDatabase()` - 11 edges
5. `六、目前比较明显的问题和风险` - 11 edges
6. `listModelsForApiKey()` - 10 edges
7. `buildPricing()` - 9 edges
8. `buildRequestPricing()` - 9 edges
9. `insertSuccessLog()` - 9 edges
10. `channelTokenOfficial()` - 9 edges
...
- `listModels()` --calls--> `getDatabase()`  [EXTRACTED]
  backend/src/routes/proxy.js → backend/src/database/init.js
...
Cohesion: 0.09
Nodes (25): { authenticate, requireAdmin }, axios, BILLING_MODEL_SOURCES, CHANNEL_BILLING_MODES, CHANNEL_MULTIPLIER_FIELDS, CHANNEL_PRICE_FIELDS, channelModelPayload(), { encrypt, desensitize } (+17 more)

### Community 9 - "pricing-engine.js"
Cohesion: 0.23
Nodes (17): buildChannelImagePricing(), buildImagePricing(), calculateDimensions(), calculateImagePricing(), calculatePricing(), configuredImageUnitPrice(), hasNumericPrice(), normalizeCurrency() (+9 more)

### Community 10 - "user.js"
Cohesion: 0.06
Nodes (46): anthropicUpstreamHeaders(), listModels(), { authenticate }, bcrypt, { buildBillingDetail }, { buildEasyPayRequest, supportedPaymentMethods }, { encrypt, decrypt, desensitize }, express (+38 more)
...
Cohesion: 0.16
Nodes (13): apiKeyAuthError(), apiKeyFromRequest(), authenticateApiKey(), bcrypt, findApiKey(), generateToken(), { getDatabase }, jwt (+5 more)
...
Cohesion: 0.27
Nodes (16): buildPricing(), buildRequestPricing(), capChatRequestToReservedBalance(), channelHasTokenPricing(), getUsdCnyRate(), insertSuccessLog(), pricingModelForChannel(), billingModeForRequest() (+8 more)
...
Cohesion: 0.60
Nodes (3): channelMultiplierPayload(), positiveMultiplier(), resolveChannelMultipliers()
...
Cohesion: 0.19
Nodes (13): availableWalletBalance(), channelBillingForModel(), getEffectiveMultiplier(), handleImageBilledRequest(), insertImageSettlementFailureLog(), insertImageSuccessLog(), insertSettlementFailureLog(), insertUpstreamFailureLog() (+5 more)
...
## Knowledge Gaps
- **545 isolated node(s):** `backup.sh script`, `name`, `version`, `main`, `start` (+540 more)
...
- **Why does `getDatabase()` connect `getDatabase` to `proxy.js`, `admin-finance.test.js`, `image-generations.test.js`, `admin.js`, `user.js`, `registration.test.js`, `middleware/auth.js`, `src/index.js`, `handleImageBilledRequest`, `anthropic-protocol.test.js`, `billing.test.js`, `easypay-payment.test.js`, `easypay.js`, `model-capabilities.test.js`?**
  _High betweenness centrality (0.020) - this node is a cross-community bridge._
- **Why does `listModelsForApiKey()` connect `user.js` to `admin.js`, `proxy.js`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
- **Why does `upstreamRequestHeaders()` connect `user.js` to `admin.js`, `proxy.js`?**
  _High betweenness centrality (0.003) - this node is a cross-community bridge._
